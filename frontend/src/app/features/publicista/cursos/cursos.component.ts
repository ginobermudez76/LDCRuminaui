...
@Component({
  ...
  template: `
    <div class="pub-container">
      <header class="page-header purple">
        <h1>📚 Cursos</h1>
        <p>Administra los cursos deportivos disponibles.</p>
      </header>
        ...
        <p-table [value]="items()" [paginator]="true" [rows]="10" styleClass="p-datatable-sm p-datatable-striped" responsiveLayout="scroll">
          <ng-template pTemplate="caption">
            <div class="table-caption">
              ...
              <button pButton type="button" icon="pi pi-plus" label="Nuevo Curso" class="p-button-sm" (click)="openDialog()"></button>
            </div>
          </ng-template>
          <ng-template pTemplate="header">
            <tr><th>ID</th><th>Imagen</th><th>Nombre</th><th>Descripción</th><th>Fecha Inicio</th><th>Fecha Fin</th><th>Estado</th><th>Acciones</th></tr>
          </ng-template>
          ...
          <ng-template pTemplate="emptymessage"><tr><td colspan="8" class="empty-msg">No hay cursos registrados.</td></tr></ng-template>
        </p-table>
      </div>
      <p-dialog [(visible)]="showDialog" header="Nuevo Curso" [modal]="true" [style]="{width:'540px'}">
        <form [formGroup]="form" (ngSubmit)="onSubmit()" class="modal-form">
          <div class="form-grid">
            ...
            <div class="field"><label>Estado</label>
              <p-select [options]="['Activo','Inactivo']" formControlName="estado" [style]="{width:'100%'}"></p-select>
            </div>
  ...
  styles: [`
    ...
    .purple { background: linear-gradient(135deg, #7c3aed 0%, #a78bfa 100%); }
...
export class CursosComponent implements OnInit {
  ...
  ngOnInit() {
    this.form = this.fb.group({ nombre: ['', Validators.required], descripcion: [''], fecha_inicio: [''], fecha_fin: [''], deporte_id: [null], estado: ['Activo'], inscripciones: ['Activo'] });
    this.load();
    this.svc.getDeportes().subscribe(d => this.deportes.set(d));
  }

  load() { this.svc.getCursos().subscribe(d => this.items.set(d)); }
  onFileChange(e: any) { this.selectedFile = e.target.files[0] || null; }
  openDialog() { this.form.reset({ estado: 'Activo', inscripciones: 'Activo' }); this.selectedFile = null; this.error.set(undefined); this.showDialog = true; }
