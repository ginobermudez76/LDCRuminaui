        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            try {
                ...
                if (isset($_POST['checkDImagen'])) {
                    eliminarArchivoYActualizarBD($idDeportista, $deporte);
                } else {
                    // Actualizar la solicitud en la base de datos sin cambiar el archivo
                    actualizarBD($idDeportista, $deporte);
                }
...
function eliminarArchivoYActualizarBD($idDeportista, $deporte) {
    global $conn;
    ...
    $stmt = $conn->prepare("UPDATE deportistas_destacados SET imagen = NULL, deporte_id = ? WHERE id = ?");
    $stmt->execute([$deporte, $idDeportista]);
}

function actualizarBD($idDeportista, $deporte) {
    global $conn;

    $stmt = $conn->prepare("UPDATE deportistas_destacados SET deporte_id = ? WHERE id = ?");
    $stmt->execute([$deporte, $idDeportista]);
}
?>