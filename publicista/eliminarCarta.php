...
try {
    ...
    if ($usuario['rol'] == 7) {
        ...
        if ($deportistaId1 && !empty($deportistaId1['imagen'])) {
            ...
            $rutaImagen = "../uploads/cartaCondolencia/" . basename($deportistaId1['imagen']);
        ...
        if (isset($_POST['id'])) {
            ...
            $stmtDeleteDeportista = $conn->prepare("DELETE FROM carta_condolencias WHERE id = :id");
            $stmtDeleteDeportista->bindParam(':id', $deportistaId);
            $stmtDeleteDeportista->execute();

            echo "Carta eliminada con éxito";
        }
    }
    } else {
